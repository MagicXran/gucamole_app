...
FIXED_WORKSPACE = r"\\tsclient\{user_drive}"


CONTROL_CATALOG: tuple[dict[str, Any], ...] = (
    {"code": "workspace_file_ops", "category": "workspace", "label": "个人资料空间文件操作", "enforcement": "Portal + NTFS", "risk": "允许在当前 Portal 用户的资料空间中新建、读写、删除和重命名文件。"},
    {"code": "multi_root_workspace", "category": "workspace", "label": "多根工作区", "enforcement": "VSCode policy", "risk": "工作区仍必须位于当前 Portal 用户的资料空间路径。"},
    ...
    {"code": "browser_upload", "category": "data_channel", "label": "浏览器上传", "enforcement": "Guacamole", "risk": "允许浏览器向当前用户资料空间上传文件。"},
    {"code": "browser_download", "category": "data_channel", "label": "浏览器下载", "enforcement": "Guacamole", "risk": "允许从当前用户资料空间下载到浏览器。"},
...
LOCKED_BASELINE = (
    ...
    {"code": "guacdrive_workspace", "label": "默认业务工作区为当前用户的资料空间"},
...
def _normalize_workspace(value: Any) -> str:
    ...
    if normalized != FIXED_WORKSPACE:
        raise VscodePolicyError("default_workspace_template 必须固定为 \\\\tsclient\\{user_drive}")
    return normalized
...
def build_vscode_arguments(profile: dict[str, Any], user_id: int, *, drive_name: str) -> str:
    ...
    workspace = profile["default_workspace_template"].replace("{user_drive}", drive_name)
    return (
