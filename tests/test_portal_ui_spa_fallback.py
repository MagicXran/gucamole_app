...
def test_index_html_redirects_to_portal():
    _require_portal_build()

    response = _request("GET", "/index.html")

    assert response.status_code in (302, 307)
    assert response.headers["location"] == "/portal"


def test_admin_html_redirects_to_vue_admin_workbench():
    _require_portal_build()

    response = _request("GET", "/admin.html")

    assert response.status_code in (302, 307)
    assert response.headers["location"] == "/portal/admin/apps"


def test_viewer_html_redirects_to_workspace_when_no_path():
    _require_portal_build()

    response = _request("GET", "/viewer.html")

    assert response.status_code in (302, 307)
    assert response.headers["location"] == "/portal/my/workspace"


def test_viewer_html_still_serves_legacy_viewer_when_path_present():
    _require_portal_build()

    response = _request("GET", "/viewer.html?path=Output/demo.vtu")

    assert response.status_code == 200
    assert "3D 仿真查看器 - VTK.js" in response.text
